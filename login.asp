...
call abreConexao
	sql = "Select * From TB_Produtor INNER JOIN TB_Propriedade ON TB_Propriedade.CNPJCPFProprietario = TB_Produtor.CNPJCPFProdutor WHERE CNPJCPFProdutor = '"&replace(replace(request.form("cpf"), ".", ""),"-", "")&"' AND Senha = '"&request.form("senha_login")&"'"
	'response.write sql
    'response.end
  set rs = conn.execute(sql)
	if not rs.eof then
    Session("CodIncra") = request.form("incra")
	Session("CPF_Usu") = replace(replace(request.form("cpf"),".",""),"-","")
    response.Redirect("frm_Cad_Alter_Suino.asp?cpf="&Session("CPF_Usu")&"&CodIncra="&Session("CodIncra")&"")
    ...
    <div class="content">      
      ...
      <div id="login">
        ...
        <input type="hidden" name="CpfVisualizar" id="CpfVisualizar" /> 
          ...
          <%
          call abreConexao
          sql = "Select * From TB_Produtor INNER JOIN TB_Propriedade ON TB_Propriedade.CNPJCPFProprietario = TB_Produtor.CNPJCPFProdutor WHERE INCRAProp = "&request.form("incra")&""
          %>
          <input type="hidden" name="incra" value="<%=(INCRAProp)%>">
          <p> 
